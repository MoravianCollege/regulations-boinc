...
def test_close_new_file(workfile_tempdir):
    wa.work_accumulator(workfile_tempdir)
    wa.end()
...
def test_end_writes(workfile_tempdir):
    wa.work_accumulator(workfile_tempdir)
    for x in range(10):
        wa.add_doc("ID", 999)
    wa.end()
...
def test_end_writes_no_extra(workfile_tempdir):
    wa.work_accumulator(workfile_tempdir)
    for x in range(10):
        wa.add_doc("ID", 1000)
    wa.end()
...
def test_single_file(workfile_tempdir):
    wa.work_accumulator(workfile_tempdir)
    wa.add_doc("ID", 999)
    assert wa.get_size() == 999
    wa.add_doc("ID", 1)
